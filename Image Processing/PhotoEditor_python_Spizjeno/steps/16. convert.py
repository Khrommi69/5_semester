...
class PyPhotoEditor:
    ...
    def convert_current_image(self, mode):
        ...
        if mode == "roll":
            if image.mode != "RGB":
                mb.showerror("RGB Roll error", f"Can't roll image with not RGB mode '{image.mode}'")
                return
            ...
            if image.mode != "RGB":
                mb.showerror("RGB Roll error", f"Can't split channel of image with not RGB mode '{image.mode}'")
                return
